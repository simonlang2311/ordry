...
export default function AdminReservationsPage() {
  ...
  useEffect(() => {
    if (!featuresLoaded || !features.reservationsEnabled) return;
    fetchReservations();
  }, [selectedDate, featuresLoaded, features.reservationsEnabled]);

  if (featuresLoaded && !features.reservationsEnabled) {
    notFound();
  }
    ...
    <div className="min-h-screen bg-app-bg text-app-text font-sans p-6 md:p-12">
      ...
      <div className="max-w-6xl mx-auto mb-8 flex flex-col gap-4 border-b border-app-muted/20 pb-6 xl:max-w-7xl">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <button
            onClick={() => router.push(`/${restaurantId}/admin`)}
        ...
        <div className="flex w-full flex-wrap items-center gap-3 bg-app-card p-3 rounded-xl border border-app-muted/20 xl:w-auto">
           <button onClick={() => router.push(`/${restaurantId}`)} className="px-4 py-2 text-sm font-bold text-app-muted hover:text-app-text transition-colors">Home</button>
           <div className="h-6 w-[1px] bg-app-muted/30"></div>
           <button onClick={() => router.push(`/${restaurantId}`)} className="px-4 py-2 text-sm font-bold text-app-muted hover:text-app-text transition-colors">
             Zum Restaurant
           </button>
           <div className="h-6 w-[1px] bg-app-muted/30"></div>
           <input 

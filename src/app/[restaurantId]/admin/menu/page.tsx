...
export default function MenuEditor() {
  ...
  const fetchData = async () => {
    ...
    // A) Gerichte laden
    const { data: menuData, error: menuError } = await supabase.from('menu').select('*').eq('restaurant_id', restaurantId).order('name', { ascending: true });
    ...
    // C) Kategorien laden
    const { data: catData } = await supabase.from('settings').select('value').eq('key', 'menu_categories').eq('restaurant_id', restaurantId).single();
    if (catData?.value) {
      try {
    ...
    const { data: allergensEnabledData } = await supabase.from('settings').select('value').eq('key', 'allergens_enabled').eq('restaurant_id', restaurantId).single();
    ...
    const { data: upsellData } = await supabase.from('settings').select('value').eq('key', 'upsell_products').eq('restaurant_id', restaurantId).single();
    if (upsellData?.value) {
      try {
  ...
  const saveItemTypeOverridesToDB = async (overrides: Record<string, MenuItemType>) => {
    await supabase
      .from('settings')
      .upsert({ restaurant_id: restaurantId, key: 'menu_item_types', value: JSON.stringify(overrides) }, { onConflict: 'key,restaurant_id' });
  };
  ...
  const saveCategoriesToDB = async (cats: Category[]) => {
    ...
    await supabase.from('settings').upsert({ restaurant_id: restaurantId, key: 'menu_categories', value: JSON.stringify(sanitizedCategories) }, { onConflict: 'key,restaurant_id' });
  };
  ...
  const saveLunchSpecialToDB = async (config: LunchSpecialConfig) => {
    ...
    const { error } = await supabase.from('settings').upsert(
      { restaurant_id: restaurantId, key: 'lunch_special', value: JSON.stringify(config) }, 
  ...
  const saveUpsellProductsToDB = async (productIds: number[]) => {
    const { error } = await supabase.from('settings').upsert(
      { restaurant_id: restaurantId, key: 'upsell_products', value: JSON.stringify(productIds) },
      { onConflict: 'key,restaurant_id' }
    );
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex gap-3">
                <Link href={`/${restaurantId}`} className="bg-app-card border border-app-muted/30 px-4 py-2 rounded-lg hover:bg-app-muted/10 font-bold transition-colors">Home</Link>
  ...
  <div className="space-y-8 px-6 pb-6">
      ...
      <div className="space-y-6 px-6 pb-6">
        ...
        {features.upsellingEnabled && <div className="bg-app-card border border-app-muted/20 rounded-2xl shadow-sm overflow-hidden">
          <button
            type="button"

...
function QRCodeContent() {
        ...
        if (data) {
          const tablesWithTokens = await Promise.all(
            data.map(async (table) => ({
              ...table,
              currentToken: await fetchCurrentTokenForTable(table.label, supabase, process.env.NEXT_PUBLIC_RESTAURANT_ID || 'demo-restaurant-1')
            }))
          );
  ...
  const regenerateQRCode = async (table: Table) => {
    ...
    try {
      const newToken = await createNewTokenForTable(table.label, supabase, process.env.NEXT_PUBLIC_RESTAURANT_ID || 'demo-restaurant-1');

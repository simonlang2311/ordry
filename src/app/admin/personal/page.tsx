...
function PersonalContent() {
  ...
  const saveLogoUrl = async (url: string) => {
    try {
      const { error } = await supabase
        .from("settings")
      .upsert({ restaurant_id: process.env.NEXT_PUBLIC_RESTAURANT_ID ?? 'demo-restaurant-1', key: "logo_url", value: url }, { onConflict: "key,restaurant_id" });
  ...
  const saveTheme = async (themeName: string) => {
    ...
    const { error } = await supabase
      .from("settings")
      .upsert({ restaurant_id: process.env.NEXT_PUBLIC_RESTAURANT_ID ?? 'demo-restaurant-1', key: "theme", value: themeName }, { onConflict: "key,restaurant_id" });
  ...
  const saveFontFamily = async (fontKey: string) => {
    ...
    const { error } = await supabase
      .from("settings")
      .upsert({ restaurant_id: process.env.NEXT_PUBLIC_RESTAURANT_ID ?? 'demo-restaurant-1', key: "font_family", value: fontKey }, { onConflict: "key,restaurant_id" });
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { error } = await supabaseNoKeepalive
        .from("settings")
        .upsert({ restaurant_id: process.env.NEXT_PUBLIC_RESTAURANT_ID ?? 'demo-restaurant-1', key: "logo_url", value: dataUrl }, { onConflict: "key,restaurant_id" });
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-8 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <a href="/" className="bg-app-card border border-app-muted/30 px-4 py-2 rounded-lg hover:bg-app-muted/10 font-bold transition-colors">
              Home
            </a>

...
export default function TablePage() {
  ...
  useEffect(() => {
    ...
    const validateAccess = async () => {
      ...
      try {
        const result = await validateAndRedirectToken(tableId, tokenFromUrl, supabase, process.env.NEXT_PUBLIC_RESTAURANT_ID || 'demo-restaurant-1');
  ...
  useEffect(() => {
    ...
    const fetchMenuData = async () => {
      ...
      try {
        ...
        const catDataPromise = supabase.from('settings').select('value').eq('key', 'menu_categories').eq('restaurant_id', process.env.NEXT_PUBLIC_RESTAURANT_ID).single();
        ...
        const linkDataPromise = supabase.from('settings').select('value').eq('key', 'restaurant_link').eq('restaurant_id', process.env.NEXT_PUBLIC_RESTAURANT_ID).single();
        ...
        const allergensEnabledPromise = supabase.from('settings').select('value').eq('key', 'allergens_enabled').eq('restaurant_id', process.env.NEXT_PUBLIC_RESTAURANT_ID).single();
        const allergensDisabledNoticePromise = supabase.from('settings').select('value').eq('key', 'allergens_disabled_notice').eq('restaurant_id', process.env.NEXT_PUBLIC_RESTAURANT_ID).single();
        ...
        const upsellProductsPromise = supabase.from('settings').select('value').eq('key', 'upsell_products').eq('restaurant_id', process.env.NEXT_PUBLIC_RESTAURANT_ID).single();
        ...
        const menuDataPromise = supabase.from('menu').select('*').eq('restaurant_id', process.env.NEXT_PUBLIC_RESTAURANT_ID);
        ...
        const lunchDataPromise = supabase.from('settings').select('value').eq('key', 'lunch_special').eq('restaurant_id', process.env.NEXT_PUBLIC_RESTAURANT_ID).single();
    ...
    const channel = supabase.channel('table-menu-updates')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'settings', filter: `restaurant_id=eq.${process.env.NEXT_PUBLIC_RESTAURANT_ID}` },
        (payload) => {
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'menu', filter: `restaurant_id=eq.${process.env.NEXT_PUBLIC_RESTAURANT_ID}` },
        () => { void fetchMenuData(); })
        .subscribe();
  ...
  const fetchOrders = async () => {
    if(!tableId) return;
    const { data } = await supabase.from('orders').select('*').eq('restaurant_id', process.env.NEXT_PUBLIC_RESTAURANT_ID).eq('table_id', tableId).neq('status', 'done').neq('status', 'paid').order('created_at', { ascending: false });
  ...
  useEffect(() => {
    ...
    fetchOrders();
    const channel = supabase.channel('table-updates').on('postgres_changes', { event: '*', schema: 'public', table: 'orders', filter: `table_id=eq.${tableId},restaurant_id=eq.${process.env.NEXT_PUBLIC_RESTAURANT_ID}` }, () => fetchOrders()).subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [tableId, tableExists]);
  ...
  const addToCart = (product: Product) => {
    ...
    const suggestions = getDynamicUpsells(product.id);
  ...
  const saveRechnung = async () => {
    ...
    if (activeSplitOrder) {
      ...
      await supabase.from('orders').insert({ table_id: tableId, items: itemsToAdd, total_price: shareTotal, status: 'pay_split' });
    }

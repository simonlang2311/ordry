...
function WaiterContent() {
  ...
  const fetchPrices = async () => {
    ...
    const { data } = await supabase.from('menu').select('id, name, price, description, item_type, allergens');
    if (data) {
      ...
      const { data: lunchData } = await supabase.from('settings').select('value').eq('key', 'lunch_special').single();
      if (lunchData?.value) {
        try {
  ...
  const loadTableData = async (tableLabel: string) => {
    ...
    const { data: orders, error } = await supabase.from('orders').select('*').eq('table_id', tableLabel).order('created_at', { ascending: false });
    ...
    const { data: history } = await supabase.from('orders').select('items').eq('table_id', tableLabel).eq('status', 'paid');
    if (history) {
      let totalRev = 0;
  ...
  const deleteTable = async (e: React.MouseEvent | null, id: number, label: string) => {
    ...
    const { data: openOrders } = await supabase.from('orders').select('id').eq('table_id', label).neq('status', 'paid');
  ...
  const syncKitchenCoursesToPickedUpForTable = async (tableLabel: string): Promise<number[]> => {
    ...
    const { error } = await supabase
      .from('settings')
      .upsert(
        { restaurant_id: process.env.NEXT_PUBLIC_RESTAURANT_ID, key: 'kitchen_course_status', value: JSON.stringify(nextMap) },
  ...
  const handleOpenDetails = async (e: React.MouseEvent | null, tableLabel: string) => {
    ...
    const tok = await fetchCurrentTokenForTable(tableLabel, supabase, process.env.NEXT_PUBLIC_RESTAURANT_ID || 'demo-restaurant-1');
    setTableToken(tok);
  ...
  const cancelOrder = async (orderId: number) => {
    ...
    await supabase.from('orders').delete().eq('restaurant_id', process.env.NEXT_PUBLIC_RESTAURANT_ID).eq('id', orderId);
  ...
  const cancelOrderItem = async (orderId: number, itemIndex: number, itemName: string, requestedQty?: number) => {
    ...
    if (newItems.length === 0) {
      ...
      await supabase.from('orders').update({ items: newItems, total_price: newTotal }).eq('restaurant_id', process.env.NEXT_PUBLIC_RESTAURANT_ID).eq('id', orderId);
    }
      ...
      for (const otherOrder of tableOrders) {
        ...
        if (orderChanged) {
          ...
          if (updatedItems.length === 0) {
            await supabase.from('orders').delete().eq('restaurant_id', process.env.NEXT_PUBLIC_RESTAURANT_ID).eq('id', otherOrder.id);
          } else {
            await supabase.from('orders').update({ items: updatedItems, total_price: otherNewTotal }).eq('restaurant_id', process.env.NEXT_PUBLIC_RESTAURANT_ID).eq('id', otherOrder.id);
          }
        }
      }
    }
                ...
                {tableToken && (
                  <div className="hidden md:block text-sm text-app-muted space-y-1">
                    ...
                    {adminToken && (
                      <div>
                        Admin‑Zugriff:&nbsp;
                        <code className="bg-app-card border border-app-muted/20 px-1 py-0.5 rounded break-all text-app-text">/table/{selectedTable}?admintoken={adminToken}</code>
                        &nbsp;(<button onClick={() => {
                          navigator.clipboard.writeText(`${window.location.origin}/table/${selectedTable}?admintoken=${adminToken}`);
                        }} className="underline">kopieren</button>)
                      </div>
                    )}
                  </div>

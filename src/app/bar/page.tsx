...
function BarContent() {
  ...
  const loadCourseStatusFromSettings = async (applyState = true): Promise<Record<string, CourseStatus>> => {
    const { data } = await supabase.from("settings").select("value").eq("key", "kitchen_course_status").eq('restaurant_id', process.env.NEXT_PUBLIC_RESTAURANT_ID).single();
    if (!data?.value) {
  ...
  const persistCourseStatusToSettings = async (nextCourseMap: Record<string, CourseStatus>) => {
    ...
    const { error } = await supabase
      .from("settings")
      .upsert({ restaurant_id: process.env.NEXT_PUBLIC_RESTAURANT_ID, key: "kitchen_course_status", value: JSON.stringify(mergedCourseMap) }, { onConflict: "restaurant_id,key" });
  ...
  useEffect(() => {
    ...
    const fetchMenuTypes = async () => {
      const { data } = await supabase.from("menu").select("id, name, item_type").eq('restaurant_id', process.env.NEXT_PUBLIC_RESTAURANT_ID);
      if (data) {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'settings', filter: `key=eq.drinks_target,restaurant_id=eq.${process.env.NEXT_PUBLIC_RESTAURANT_ID}` }, () => {
        fetchDrinksTarget();
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'settings', filter: `key=eq.kitchen_course_status,restaurant_id=eq.${process.env.NEXT_PUBLIC_RESTAURANT_ID}` }, () => {
        loadCourseStatusFromSettings();
      })
      .subscribe();
  ...
  const updateOrderStatusFromCourseMap = async (
    ...
    await supabase.from("orders").update({ status: targetStatus }).eq("id", orderId).eq('restaurant_id', process.env.NEXT_PUBLIC_RESTAURANT_ID);
  };
    ...
    <div className="min-h-screen bg-app-bg p-3 sm:p-4 md:p-6 text-app-text font-sans overflow-x-hidden">
      <div className="sticky top-0 z-20 -mx-3 mb-4 flex items-center justify-between gap-3 border-b border-app-muted/20 bg-app-bg/95 px-3 pb-3 pt-1 backdrop-blur sm:-mx-4 sm:px-4 md:static md:mx-0 md:bg-transparent md:px-0 md:pt-0">
        ...
        <div className="flex shrink-0 gap-2">
          <a href="/" className="flex h-10 items-center text-sm bg-app-card text-app-text border border-app-muted/30 px-4 rounded hover:bg-app-muted/20 transition-colors">Home</a>
